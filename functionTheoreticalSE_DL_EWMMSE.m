function SE_Theoretical = functionTheoreticalSE_DL_EWMMSE( R,HMean,M,K,L,p,tau_p,prelogFactor,rho)
%Computes the DL SE with EW-MMSE estimator in Theorem 5
%Note that the covariance matrices and the mean vectors are same for all
%channel realizations.
%
%This Matlab function was developed to generate simulation results to:
%
%Ozgecan Ozdogan, Emil Bjornson, Erik G. Larsson, “Massive MIMO with
%Spatially Correlated Rician Fading Channels,” IEEE Transactions on
%Communications, To appear.
%
%Download article: https://arxiv.org/abs/1805.07972
%
%This is version 1.0 (Last edited: 2019-02-01)
%
%License: This code is licensed under the GPLv2 license. If you in any way
%use this code for research that results in publications, please cite our
%original article listed above.


%Store identity matrix of size M x M
eyeM = eye(M);

%Prepare to store  (47) and (48)
CCterm1=zeros(K,L);
CCterm2=zeros(K,L);
CCterm2_p1=zeros(K,L,L,K);
CCterm3=zeros(K,L);

%Prepare to store DL SE
SE_Theoretical=zeros(K,L);


% Go through all BSs
for j = 1:L
    
    %Go through all UEs
    for k = 1:K
        
        %Compute the matrix that is inverted in the EWMMSE estimator
        PsiInv = (p*tau_p*sum(R(:,:,k,:,j),4) +eyeM);
        LambdaInv=diag(diag(PsiInv));
        
        %Taking the diagonal elements of the covariance matrices
        Djk=diag(diag(R(:,:,k,j,j)));
        
        %Compute (47) in Theorem 5
        CCterm1(k,j)=p*tau_p*trace(Djk/LambdaInv*Djk) + norm(HMean(:,k,j,j))^2;
        Sigmajk=p*tau_p*Djk/LambdaInv*PsiInv/LambdaInv*Djk;
        CCterm3(k,j)=trace(Sigmajk)+ norm(HMean(:,k,j,j))^2;
        
        %Go through all UEs
        for lx=1:L
            for i=1:K
                
                %Calculate (49) in Theorem 5
                %Compute the common terms for non-pilot contaminated and pilot contaminated UEs
                PsiInv_li = (p*tau_p*sum(R(:,:,i,:,lx),4) +eyeM);
                Lambda_li_Inv=diag(diag(PsiInv_li));
                Dli=diag(diag(R(:,:,i,lx,lx)));
                Sigmali=p*tau_p*Dli/Lambda_li_Inv*PsiInv_li/Lambda_li_Inv*Dli;
                mu_li=trace(Sigmali)+ norm(HMean(:,i,lx,lx))^2;
                
                CCterm2_p1(k,j,lx,i)=(trace(R(:,:,k,j,lx)*Sigmali) +...
                    HMean(:,k,j,lx)'*Sigmali*HMean(:,k,j,lx)+...
                    HMean(:,i,lx,lx)'*R(:,:,k,j,lx)*HMean(:,i,lx,lx)+...
                    abs(HMean(:,i,lx,lx)'*HMean(:,k,j,lx))^2)/mu_li;
                if k==i %Compute the terms for pilot contaminated UEs
                    CCterm2_p1(k,j,lx,i)= CCterm2_p1(k,j,lx,i)+ (p*p*tau_p*tau_p*abs(trace(R(:,:,k,j,lx)/Lambda_li_Inv*Dli))^2+...
                        2*p*tau_p*real(trace(R(:,:,k,j,lx)/Lambda_li_Inv*Dli)*HMean(:,i,lx,lx)'*HMean(:,k,j,lx)))/mu_li;
                end
                
            end
        end
        
        %Sum all the interferences from all UEs
        CCterm2(k,j)=sum(sum(CCterm2_p1(k,j,:,:),3),4);
    end
end


%Compute the SE with (49) for each UE
for k=1:K
    for j=1:L
        
        SE_Theoretical(k,j)= prelogFactor*real(log2(1+ (((rho*abs(CCterm1(k,j))^2)/(CCterm3(k,j)))/(rho*CCterm2(k,j) -((rho*abs(CCterm1(k,j))^2)/(CCterm3(k,j))) + 1))));
    end
end

end

